import type { ActivityEntry, BackupBundle, ExportRecord, Project, Recording, Transcript, UserSettings } from '../types';
const DB_NAME = 'silencio-db';
const openDb = (): Promise<IDBDatabase> => new Promise((resolve, reject) => { const request = indexedDB.open(DB_NAME, 2); request.onupgradeneeded = () => { const db = request.result; if (!db.objectStoreNames.contains('recordings')) db.createObjectStore('recordings', { keyPath: 'id' }); if (!db.objectStoreNames.contains('transcripts')) db.createObjectStore('transcripts', { keyPath: 'id' }); if (!db.objectStoreNames.contains('settings')) db.createObjectStore('settings', { keyPath: 'id' }); if (!db.objectStoreNames.contains('projects')) db.createObjectStore('projects', { keyPath: 'id' }); if (!db.objectStoreNames.contains('activities')) db.createObjectStore('activities', { keyPath: 'id' }); if (!db.objectStoreNames.contains('exports')) db.createObjectStore('exports', { keyPath: 'id' }); }; request.onsuccess = () => resolve(request.result); request.onerror = () => reject(request.error); });
...
export const storageService = {
  ...
  async exportBackup(): Promise<BackupBundle> {
    const [recordings, transcripts, projects, activities, settings] = await Promise.all([
      this.getRecordings(),
      this.getTranscripts(),
      this.getProjects(),
      this.getActivities(),
